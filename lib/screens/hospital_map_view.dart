...
class _HospitalMapViewState extends State<HospitalMapView> {
  ...
  Widget _buildDepartmentZone(String dept, double leftPct, double topPct, double widthPct, double heightPct) {
...
class ClinicalMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void drawRoom(Rect rect) {
      canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(8)), roomFill);
      canvas.drawRRect(RRect.fromRectAndRadius(rect, const Radius.circular(8)), linePaint);
      
      // Simple clinical blueprint lines
      canvas.drawLine(Offset(rect.left, rect.top + 20), Offset(rect.right, rect.top + 20), linePaint);
    }

    // Departments
    drawRoom(Rect.fromLTWH(size.width * 0.1, size.height * 0.1, size.width * 0.35, size.height * 0.25)); 
    drawRoom(Rect.fromLTWH(size.width * 0.55, size.height * 0.1, size.width * 0.35, size.height * 0.3)); 
    drawRoom(Rect.fromLTWH(size.width * 0.25, size.height * 0.5, size.width * 0.5, size.height * 0.38)); 

    // Hallway Indicators
    canvas.drawLine(Offset(size.width * 0.45, size.height * 0.2), Offset(size.width * 0.55, size.height * 0.2), linePaint);
    canvas.drawLine(Offset(size.width * 0.5, size.height * 0.35), Offset(size.width * 0.5, size.height * 0.5), linePaint);
  }
...
class _EquipmentListPanelState extends State<_EquipmentListPanel> with SingleTickerProviderStateMixin {
  ...
  void _showEquipmentForm(BuildContext context, [HospitalAsset? existing]) {
    showModalBottomSheet(
      ...
      builder: (ctx) => _EquipmentEntryForm(
        ...
        onComplete: () { Navigator.pop(ctx); setState((){}); }
      ),
    );
  }
    ...
    return SizedBox(
      ...
      child: Column(
        children: [
            ...
            child: Row(
              children: [
                Column(
                  ...
                  children: [
                    Text(widget.department, style: Theme.of(context).textTheme.titleLarge),
                    const Text("Equipment List", style: TextStyle(color: AppTheme.textSecondary, fontSize: 13)),
                  ],
                ),
                const Spacer(),
          ...
          Expanded(
            child: FutureBuilder<List<HospitalAsset>>(
              future: AssetService.instance.getAssetsByUnit(widget.department.toUpperCase()),
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
                final assets = snapshot.data!.where((a) => a.assetType == (_tabCtrl.index == 0 ? 'ventilator' : 'anaesthetic_machine')).toList();

                if (assets.isEmpty) return const Center(child: Text("No equipment found in this department."));
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    Color statusColor = asset.status == 'OPERATIONAL' ? AppTheme.success : (asset.status == 'MAINTENANCE' ? AppTheme.warning : AppTheme.error);

                    return Card(
                      margin: const EdgeInsets.only(bottom: 12),
                      child: ListTile(
                        onTap: () => _showEquipmentForm(context, asset),
                        title: Text(asset.modelName, style: const TextStyle(fontWeight: FontWeight.bold)),
...
class _EquipmentEntryForm extends StatefulWidget {
  ...
  const _EquipmentEntryForm({required this.department, required this.assetType, this.existingAsset, required this.onComplete});
...
class _EquipmentEntryFormState extends State<_EquipmentEntryForm> {
  late TextEditingController _modelCtrl, _serialCtrl, _wardCtrl, _svcCtrl, _intCtrl;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(widget.existingAsset == null ? "Add Equipment" : "Update Details", 
            style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 20),
          TextField(controller: _modelCtrl, decoration: const InputDecoration(labelText: "Model Name")),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(child: TextField(controller: _serialCtrl, decoration: const InputDecoration(labelText: "Serial Number"))),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: _wardCtrl, decoration: const InputDecoration(labelText: "Room/Ward"))),
            ],
          ),
          const SizedBox(height: 16),
          ...
          Row(
            children: [
              Expanded(child: TextField(controller: _svcCtrl, decoration: const InputDecoration(labelText: "Last Service"))),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: _intCtrl, decoration: const InputDecoration(labelText: "Interval"))),
            ],
          ),
            ...
            child: ElevatedButton(
              onPressed: () async {
                final asset = HospitalAsset(
                  ...
                  status: _status,
                  dateAcquired: widget.existingAsset?.dateAcquired ?? '2024-01-01',
